...
void setup() {
  ...
  // Definir pines PC_6 y PC_7 como los que definen la canción que se reproducirá desde el ESP32
  pinMode(MUSICA_1, OUTPUT);
  pinMode(MUSICA_2, OUTPUT);
  
  SysCtlClockSet(SYSCTL_SYSDIV_2_5 | SYSCTL_USE_PLL | SYSCTL_OSC_MAIN | SYSCTL_XTAL_16MHZ);
  // Declarar comunicación interna, comunicación UART con la otra Tiva y SPI con el lector de SD
  Serial.begin(115200);
  ...
  // Leer imágen de menú principal desde la SD
...
void loop() {
  // Reproducir la canción 1 del ESP32 en el menú principal
  digitalWrite(MUSICA_1, 1);
  digitalWrite(MUSICA_2, 0);
  
  // Realizar lecturas constantes del botón y del joystick dentro del menú
  control = analogRead(PE_0);
  pushButton = digitalRead(PF_4);
  
  // Switch que define dónde está el usuario dentro de las 3 opciones del menú
  switch(opcionMenu){
    // Para el caso 0, el usuario está sobre la opción de modo 1 jugador
    case 0:
      // Mostrar sprite de pierna de pollo que actúa como flecha de selección sobre modo 1 jugador
    ...
    // Para case 1, el usuario está sobre la opción de modo 2 jugadores
    case 1:
      // Mostrar sprite de pierna de pollo que actúa como flecha de selección sobre modo 2 jugadores
      animPierna = (globalCont / 150) % 2;
    ...
    // Para case 2, el usuario está sobre modo créditos
    case 2:
      // Mostrar sprite de pierna de pollo que actúa como flecha de selección sobre modo créditos
      animPierna = (globalCont / 150) % 2;
      LCD_Sprite(86, 115, 25, 12, menuPierna1, 2, 1, 0, 0);
      LCD_Sprite(86, 139, 25, 12, menuPierna2, 2, 1, 0, 0);
  ...
  // Si se decide entrar al modo 1 jugador desde el menú, ejecutar este código una vez
  if(modoP1 == 1){
    // Dibujar el cielo
  ...
  // Mientras se esté en modo 1 jugador, ejecutar
  while(modoP1 == 1){
    ...
    if(controlAvailable == true){
      if(control<0+50){
        // Condición de ramiro agachado
        estadoRamiro = 0;
      } else if(control<4095-50){
        // Condición de ramiro caminando normal
        estadoRamiro = 1;
      } else{
        // Condición de ramiro saltando
        estadoRamiro = 2;
    ...
    // Switch que controla el enemigo que se encuentra en pantalla actualmente
    switch(enemyIndex){
      //Generar chancla
      case 0:
        // Revisar la colisión
        colisionTest = collision(50, posRamiroY, 32, 35, posChanclaX, 240-32-36-25, 36, 36);
        // Si la chancla llegó al final de la pantalla, desaparecerla, resetear su posición, y desactivar temporalmente su movimiento
        if(posChanclaX == 0){
          FillRect(0, 0, 48, 240-32, 0x5C9F);
          posChanclaX = 320-40;
          enemyIndex = 3;
        // Mientras no halla llegado al final de la pantalla, mover la chancla hacia la izquierda mientras gira
        ...
        // Si se entra en un estado de posición prohibido, retornarlo a 0
        } else {
          posChanclaX = 0;
        }
      break;
      ...
      case 1:
        ...
        // Si la sartén llegó al final de la pantalla, desaparecerla, resetear su posición, y desactivar temporalmente su movimiento
        if(posSartenX == 0){
          FillRect(0, 0, 48, 240-32, 0x5C9F);
          posSartenX = 320-45;
          enemyIndex = 3;
        // Mientras no halla llegado al final de la pantalla, mover la sartén a la izquierda mientras está animada
      ...
      colisionTest = collision(50, posRamiroY, 32, 35, posOllaX, 240-32-52, 44, 52);
        // Si la olla llegó al final de la pantalla, desaparecerla, resetear su posición, y desactivar temporalmente su movimiento
        if(posOllaX == 0){
          FillRect(0, 0, 52, 240-32, 0x5C9F);
          posOllaX = 320-45;
          enemyIndex = 3;
        // Mientras no halla llegado al final de la pantalla, mover la olla hacia la izquierda mientras está animada
        } else if (posOllaX > -1){
      ...
      // Default en el que no se ejecutan animaciones, estado en el que se encuentra el programa cuando se espera a generar otro enemigo
      default:
      break;
    }
    ...
    if(deltaMillis > 2000){
      ...
      // Actualizar la variable de tipo de enemigo que será generado
      enemyIndex = millisNuevo % 3;
    }

    // Si existe una colisión entre Ramiro y un enemigo, entrar
    if(colisionTest == 1){
      // Desplegar imágen y texto de derrota
      LCD_Bitmap(88, 48, 144, 144, sopaDerrota);
      LCD_Print(text1, 88, 48, 2, 0xEF9F, 0xC34D);
      // Apagar la música
      digitalWrite(MUSICA_1, 0);
      digitalWrite(MUSICA_2, 0);
      // Encerrar en ciclo while hasta que el usuario presione su push button, entonces regresa al menú principal
      while(true){
        pushButton = digitalRead(PF_4);
        if(pushButton == 0){
          modoP1 = false;
  ...
  // Si se decide entrar al modo 2 jugadores desde el menú, ejecutar este código una vez
  if(modoP2 == true){
    // Enviar pulso para activar el juego en la otra pantalla por comunicación serial
    uartTX = '1';
  ...
  // Mientras se esté en modo 2 jugadores, ejecutar
  while(modoP2 == true){
  ...
  // Si se decide entrar al modo créditos desde el menú, ejecutar este código una vez
  if(modoCreditos == true){
    // Imprimir imágen de créditos desde la SD
  ...
  // Mientras se esté en modo créditos, ejecutar
  ...
  // Actualizar el contador global independientemente del modo en el que esté el usuario
  globalCont++;
}
//***************************************************************************************************************************************
...
//***************************************************************************************************************************************
//  SD Bitmap Menu (Se muestra el bitmap que contiene la imágen principal del menú desde la SD)
//***************************************************************************************************************************************
...
//***************************************************************************************************************************************
//  SD Bitmap Creditos (Se muestra el bitmap que contiene la imágen principal de los créditos desde la SD)
//***************************************************************************************************************************************
...
//***************************************************************************************************************************************
// Función para verificar los archivos de la SD, no esencial para el funcionamiento del código
//***************************************************************************************************************************************
void showFiles() {
...
//***************************************************************************************************************************************
// Función para determinar si el personaje principal sufre una colisión con cualquiera de los enemigos
//***************************************************************************************************************************************
